...
import { AnalyzeImagesResponseSchema } from "../../types/analyze";
 ...
 * - 錯誤響應格式化
 */
export class AnalyzeImages extends OpenAPIRoute {
  public schema = {
    tags: ["Gemini AI"],
    summary: "分析餐點圖片",
    description:
      "使用 Gemini AI 分析用戶上傳的餐點圖片，返回詳細的營養資訊和健康評估",
          ...
          "multipart/form-data": {
            schema: z.object({
              images: z.union([
                ...
                description: "餐點圖片文件（支援 JPEG、PNG、GIF、WebP），最多 5 張",
                minItems: 1,
                maxItems: 5,
              }),
              user_language: z.string().optional().default("zh_TW").openapi({
    ...
    responses: {
      "200": {
        ...
        content: {
          "application/json": {
            schema: AnalyzeImagesResponseSchema.openapi({
              description: "圖片分析結果",
            }),
          },
        },
  ...
  async handle(c: AppContext) {
    try {
      ...
      for (const fileData of imageFilesData) {
        if (fileData && typeof fileData === 'object' && 'name' in fileData && 'size' in fileData && 'type' in fileData) {
      ...
      const userLanguage = userLanguageData ? String(userLanguageData) : "zh_TW";
      ...
      // 7. 驗證用戶輸入
      const inputValidationError = geminiController.validateUserInput(userInput);
      ...
      const result = await geminiController.analyzeImages(
        imageFiles,
        ...
        return c.json(
          {
            ...
            errors: [
              {
                code: 400,
                message: result.error || "分析圖片時發生錯誤",
              },
            ],
          },
          400
      ...
      console.error("AnalyzeImages endpoint - 處理請求時發生錯誤:", error);
      ...
      if (
        ...
        return c.json(
          {
            success: false,
            errors: [{ code: 413, message: "圖片文件過大，請上傳小於 10MB 的圖片" }],
          },
          413 as any
        );
      }
      ...
      if (
        ...
        return c.json(
          {
            success: false,
            errors: [{ code: 400, message: "請求格式錯誤，請使用 multipart/form-data 格式上傳圖片" }],
          },
          400 as any
        );
      }
