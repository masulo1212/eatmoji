...
export const firestoreTimestampToDate = (timestamp: any): Date => {
  ...
  console.warn(`Invalid timestamp format: ${typeof timestamp}, using current date`);
  return new Date();
};
...
export const OptionalFirestoreDateSchema = z.any().optional().transform((val) => {
...
// Schema for updating a diary (all fields optional except id)
export const UpdateDiarySchema = DiarySchema.partial().extend({
  id: z.string()
});
