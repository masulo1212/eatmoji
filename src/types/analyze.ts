      ...
      items: {
        ...
        properties: {
          name: {
            type: "string",
            description:
              "食材名稱，使用用戶指定的語言（不是中文，除非用戶語言為中文），簡潔不含動詞或裝飾詞，不含括號註解，不要使用逗號做補充",
          },
          engName: {
            type: "string",
            description:
              "食材英文名稱，優先使用單數形式，但若該食材的標準英文表達為複數則使用複數（如 'noodles', 'beans', 'oats'），簡潔不含動詞或裝飾詞，不含括號註解，不要使用逗號做補充",
          },
          calories: {
            type: "number",
            description: "該食材的熱量",
...
export const addRecipeJsonSchema = {
  ...
  properties: {
    name: {
      type: "object",
      description:
        "食譜名稱（多語言），簡潔不含動詞或裝飾詞，不含括號註解，不要使用逗號做補充",
      properties: {
      ...
      items: {
        ...
        properties: {
          name: {
            type: "object",
            description:
              "食材名稱（多語言），簡潔不含動詞或裝飾詞，不含括號註解，不要使用逗號做補充",
            properties: {
              zh_TW: { type: "string", description: "繁體中文" },
              zh_CN: { type: "string", description: "简体中文" },
              en: {
          ...
          calories: {
            type: "number",
            description:
              "該食材的熱量，必須等於 (protein × 4) + (carbs × 4) + (fat × 9)",
            minimum: 0,
          },
          protein: {
            type: "number",
